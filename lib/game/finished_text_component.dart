...
class FinishedTextComponent extends TextComponent
    with HasGameRef<NeonRevenantGame> {
  double _opacity = 0.0;
  bool _isFadingIn = true;
  bool _isFadingOut = false;
  final double fadeSpeed = 2.0;
  ...
  void update(double dt) {
    ...
    if (_isFadingIn) {
      ...
      if (_opacity <= 0.0) {
        _opacity = 0.0;
        // Volver al mapa inicial después de que el texto desaparezca
        gameRef.loadMap('littleroot');
        removeFromParent();
      }
    }
    ...
    textRenderer = TextPaint(
      ...
      ).copyWith(color: Colors.green.withOpacity(_opacity)),
    );
  }
}
